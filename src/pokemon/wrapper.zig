    ...
    pub fn getTrainerPokemon(wrapper: &const Gen3, trainer: &const gen3.Trainer, index: usize) -> %?&gen3.PartyMember {
        ...
        switch (trainer.party_type) {
            gen3.PartyType.Standard => {
                const party = try getParty(gen3.PartyMember, trainer_parties, offset, trainer.party_size.get(), party_table_end);
                return utils.ptrAt(gen3.PartyMember, party, index);
            },
            gen3.PartyType.WithMoves => {
                const party = try getParty(gen3.PartyMemberWithMoves, trainer_parties, offset, trainer.party_size.get(), party_table_end);
                const pokemon = utils.ptrAt(gen3.PartyMemberWithMoves, party, index) ?? return null;
                return &pokemon.base;
            },
            gen3.PartyType.WithHeld => {
                const party = try getParty(gen3.PartyMemberWithHeld, trainer_parties, offset, trainer.party_size.get(), party_table_end);
                const pokemon = utils.ptrAt(gen3.PartyMemberWithHeld, party, index) ?? return null;
                return &pokemon.base;
            },
            gen3.PartyType.WithBoth => {
                const party = try getParty(gen3.PartyMemberWithBoth, trainer_parties, offset, trainer.party_size.get(), party_table_end);
